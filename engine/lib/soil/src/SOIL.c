	...
	)
{
	...
	if( (x_pos_file == NULL) ||
		...
		(z_neg_file == NULL) )
	{
		result_string_pointer = "Invalid light map files list";
		return 0;
	}
	/*	capability checking	*/
	if( query_cubemap_capability() != SOIL_CAPABILITY_PRESENT )
	{
		result_string_pointer = "No light map capability present";
		return 0;
	}
	...
	)
{
	...
	if( (x_pos_buffer == NULL) ||
		...
		(z_neg_buffer == NULL) )
	{
		result_string_pointer = "Invalid light map buffers list";
		return 0;
	}
	/*	capability checking	*/
	if( query_cubemap_capability() != SOIL_CAPABILITY_PRESENT )
	...
	)
{
	...
	if( filename == NULL )
	{
		result_string_pointer = "Invalid single light map file name";
		return 0;
	}
	/*	does the user want direct uploading of the image as a DDS file?	*/
	if( flags & SOIL_FLAG_DDS_LOAD_DIRECT )
	...
	for( i = 0; i < 6; ++i )
	{
		if( (face_order[i] != 'N') &&
			...
			(face_order[i] != 'D') )
		{
			result_string_pointer = "Invalid single light map face order";
			return 0;
		};
	}
	/*	capability checking	*/
	...
	)
{
	...
	if( buffer == NULL )
	{
		result_string_pointer = "Invalid single light map buffer";
		return 0;
	}
	/*	does the user want direct uploading of the image as a DDS file?	*/
	if( flags & SOIL_FLAG_DDS_LOAD_DIRECT )
	...
	)
{
	...
	if( data == NULL )
	{
		result_string_pointer = "Invalid single light map image data";
		return 0;
	}
	/*	face order checking	*/
	for( i = 0; i < 6; ++i )
	...
	for( i = 0; i < 6; ++i )
	{
		...
		/*	what is my texture target?
			...
			LHS if viewed from inside the light!	*/
		switch( face_order[i] )
		{
		case 'N':
